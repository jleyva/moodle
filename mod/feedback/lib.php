...
function feedback_check_updates_since(cm_info $cm, $from, $filter = array()) {
    ...
    $updates->submissionscompleted = (object) array('updated' => false);
    $updates->submissionsuncompleted = (object) array('updated' => false);
    $select = 'feedback = ? AND userid = ? AND timemodified > ?';
    $params = array($cm->instance, $USER->id, $from);

    $submissionscompleted = $DB->get_records_select('feedback_completed', $select, $params, '', 'id');
    if (!empty($submissionscompleted)) {
        $updates->submissionscompleted->updated = true;
        $updates->submissionscompleted->itemids = array_keys($submissionscompleted);
    }
    $submissionsuncompleted = $DB->get_records_select('feedback_completedtmp', $select, $params, '', 'id');
    if (!empty($submissionsuncompleted)) {
        $updates->submissionsuncompleted->updated = true;
        $updates->submissionsuncompleted->itemids = array_keys($submissionsuncompleted);
    }
