...
class mod_feedback_external extends external_api {
        ...
        if (!empty($params['courseids'])) {
            ...
            foreach ($feedbacks as $feedback) {
                ...
                if (!has_capability('mod/feedback:edititems', $context)) {
                    $optionals = feedback_summary_exporter::get_optional_properties();
                    foreach ($optionals as $optional) {
                        unset($feedback->{$optional});
                    }
                }
     ...
     * @since  Moodle 3.3
     */
    protected static function validate_feedback_access(feedback $feedback,  $course, $cm, $context, $checksubmit = false) {
    ...
    public static function get_analysis($feedbackid, $groupid = 0) {
        ...
        // Check permissions.
        $feedbackstructure = new mod_feedback_structure($feedback->to_record(), $cm);
        ...
        if ($checkanonymously) {
            ...
            foreach ($items as $item) {
                ...
                $analysisdata = $itemobj->get_analysed_for_external($item, $groupid);
                $itemsdata[] = array(
                    'name' => external_format_string($item->name, $context->id),
                    'label' => $item->label,
                    'type' => $item->typ,
                    'data' => $analysisdata
                );
            ...
            throw new moodle_exception('insufficient_responses_for_this_group', 'feedback');
        }
    ...
    public static function get_analysis_returns() {
        return new external_single_structure(
            ...
            'itemsdata' => new external_multiple_structure(
                new external_single_structure(
                    array(
                        'name' => new external_value(PARAM_RAW, 'The item (question name).'),
                        'label' => new external_value(PARAM_NOTAGS, 'The item label.'),
                        'type' => new external_value(PARAM_ALPHA, 'The item type.'),
                        'data' => new external_multiple_structure(
     ...
     * @param int $sort sort param, must be firstname, lastname or lastaccess (default)
    ...
    /**
     * Describes the parameters for get_responses.
     *
     * @return external_function_parameters
     * @since Moodle 3.3
     */
     ...
     * @param int $page the page of records to return
     * @param int $perpage the number of records to return per page
     * @return array of warnings and users responses
     * @since Moodle 3.3
     */
    public static function get_responses($feedbackid, $groupid = 0, $page = 0, $perpage = 0) {

        $params = array('feedbackid' => $feedbackid, 'groupid' => $groupid, 'page' => $page, 'perpage' => $perpage);
        $params = self::validate_parameters(self::get_responses_parameters(), $params);
        $warnings = $itemsdata = array();
        ...
        $result = array(
            'responses'          => $responsestable->export_external_structure($params['page'], $params['perpage']),
            'totalresponses'     => $responsestable->get_total_responses_count(),
            'anonresponses'      => $anonresponsestable->export_external_structure($params['page'], $params['perpage']),
            'totalanonresponses' => $anonresponsestable->get_total_responses_count(),
            'warnings'       => $warnings
        );
        return $result;
    }
    ...
    /**
     * Describes the get_responses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.3
     */
    public static function get_responses_returns() {
        ...
        return new external_single_structure(
            array(
                ...
                'totalresponses' => new external_value(PARAM_INT, 'Total responses count.'),
                'anonresponses' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'id' => new external_value(PARAM_INT, 'Completed id'),
                            'courseid' => new external_value(PARAM_INT, 'Course id'),
                ...
                'totalanonresponses' => new external_value(PARAM_INT, 'Total anonymous responses count.'),
                'warnings' => new external_warnings(),
            )
        );
    }
