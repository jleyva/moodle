...
class mod_quiz_external extends external_api {
     ...
     * @since  Moodle 3.1
     */
    protected static function validate_attempt($params, $checkaccessrules = true) {
        global $USER;
        ...
        if (isset($params['page'])) {
            ...
            quiz_check_page_access($attemptobj, $params['page'], false);
     ...
     * @since Moodle 3.1
     */
    public static function save_attempt($attemptid, $data) {
        global $DB;
     ...
     * @since Moodle 3.1
     */
    public static function process_attempt($attemptid, $data, $finishattempt = false, $timeup = false) {
        ...
        $result = array();
        $result['state'] = quiz_process_attempt($attemptobj, $timenow, $finishattempt, $timeup, 0);
        $result['warnings'] = $warnings;
        return $result;
    }
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
            array(
                'rescaledgrade' => new external_value(PARAM_RAW, 'rescaled grade for the quiz (or empty or "notyetgraded")'),
                'attempt' => self::attempt_structure(),
    ...
    public static function view_attempt($attemptid, $page) {
        ...
        // Log action.
        quiz_view_attempt($attemptobj);

        // Update attempt page, throwing an exception if $page is not valid.
        quiz_set_attempt_page($attemptobj, $params['page']);
    ...
    public static function view_attempt_summary($attemptid) {
        ...
        // Log action.
        quiz_view_attempt_summary($attemptobj);
    ...
    public static function view_attempt_review($attemptid) {
        ...
        // Log action.
        quiz_view_attempt_review($attemptobj);
