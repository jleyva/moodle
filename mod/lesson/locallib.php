...
function lesson_get_user_detailed_report_data(lesson $lesson, $userid, $attempt) {
    ...
    if (!empty($userid)) {
        ...
        if ($alreadycompleted) {
            ...
            $userstats->gradeinfo = lesson_grade($lesson, $attempt, $user->id);
        }
    }
...
class lesson extends lesson_base {
    ...
    /**
     * Course module object gets set and retrieved by
     * {@see get_cm()} by directly calling <code>$lesson->cm;</code>
     * @var stdClass
     */
    protected $cm = null;
     ...
     *
     * @param  [type] $lessonattempt the lesson attempt number (also known as retries)
     * @param  [type] $userid        the user id to retrieve the data from
     ...
     *
     * @param stdClass course module objct
     * @since  Moodle 3.3
     */
     ...
     * @since  Moodle 3.3
     */
    public function add_messages_on_page_view(lesson_page $page) {
        global $DB, $USER;
     ...
     */
    public function prepare_page_and_contents($pageid, $lessonoutput) {
        global $USER, $CFG;
        ...
        // Add different informative messages to the given page.
        $this->add_messages_on_page_view($page);
    ...
    public function calculate_new_page_on_jump(lesson_page $page, $newpageid) {
        ...
        if (isset($USER->modattempts[$this->properties->id])) {
            ...
            if ($newpageid = $newpage->override_next_page($newpageid)) {
                $newpageid = $newpageid;
            }
            ...
            if ($canmanage) {
                ...
                $newpageid = $lesson->cluster_jump($page->id);
            }
        } else if ($newpageid == 0) {
            $newpageid = $page->id;
