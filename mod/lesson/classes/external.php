...
class mod_lesson_external extends external_api {
     ...
     */
    public static function get_access_information($lessonid) {
        global $DB, $USER;
        ...
        $capabilitiestocheck = ['addinstance', 'edit', 'grade', 'viewreports', 'manage', 'manageoverrides'];
        $usercapabilities = [];
        foreach ($capabilitiestocheck as $cap) {
            $capname = 'mod/lesson:' . $cap;
            $usercapabilities[] = [
                'name' => $capname,
                'enabled' => has_capability($capname, $context),
            ];
        }
