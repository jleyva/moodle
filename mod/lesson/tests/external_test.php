...
class mod_lesson_external_testcase extends externallib_advanced_testcase {
    ...
    /**
     * Test the get_access_information function.
     */
    public function test_get_access_information() {
        ...
        $result = mod_lesson_external::get_access_information($this->lesson->id);
        $result = external_api::clean_returnvalue(mod_lesson_external::get_access_information_returns(), $result);
        $this->assertFalse($result['canmanage']);
        foreach ($result['capabilities'] as $cap) {
            $this->assertFalse($cap['enabled']);
        }
