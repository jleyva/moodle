...
class mod_lesson_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_get_lesson_access_information() {
        ...
        $this->assertEquals(1, $result['retries']);
    ...
    /**
     * Test for get_page_attempts
     */
    public function test_get_page_attempts() {
        global $DB;
        ...
        $result = mod_lesson_external::get_page_attempts($this->lesson->id, $attemptnumber);
        $result = external_api::clean_returnvalue(mod_lesson_external::get_page_attempts_returns(), $result);
        $this->assertCount(0, $result['warnings']);
        ...
        $result = mod_lesson_external::get_page_attempts($this->lesson->id, $attemptnumber, true);
        $result = external_api::clean_returnvalue(mod_lesson_external::get_page_attempts_returns(), $result);
        $this->assertCount(0, $result['warnings']);
        $this->assertCount(1, $result['attempts']);

        // Test filtering. Only correct only for page 2.
        $result = mod_lesson_external::get_page_attempts($this->lesson->id, $attemptnumber, true, $this->page2->id);
        $result = external_api::clean_returnvalue(mod_lesson_external::get_page_attempts_returns(), $result);
        $this->assertCount(0, $result['warnings']);
        $this->assertCount(1, $result['attempts']);
        ...
        $result = mod_lesson_external::get_page_attempts($this->lesson->id, $attemptnumber, false, null, $this->student->id);
        $result = external_api::clean_returnvalue(mod_lesson_external::get_page_attempts_returns(), $result);
        $this->assertCount(0, $result['warnings']);
        $this->assertCount(1, $result['attempts']);
        ...
        $result = mod_lesson_external::get_page_attempts($this->lesson->id, $attemptnumber, false, null, $this->teacher->id);
    }
    ...
    public function test_finish_attempt_not_doing_anything() {
        ...
        $this->assertCount(0, $result['warnings']);
        $this->assertEquals(0, $result['progress']);    // Nothing done!
        $returneddata = [];
