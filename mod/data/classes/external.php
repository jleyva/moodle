...
class mod_data_external extends external_api {
    ...
    /**
     * eturn one entry record from the database, including contents optionally.
     *
     ...
     * @throws moodle_exception
     */
    public static function get_entry($entryid, $groupid = 0, $returncontents = false) {
        global $PAGE, $DB;

        $params = array('entryid' => $entryid, 'groupid' => $groupid, 'returncontents' => $returncontents);
        ...
        // Check correct record entry.
        if (!data_can_view_record($database, $record, $currentgroup, $canmanageentries)) {
     ...
     * @throws moodle_exception
     */
    public static function search_entries($databaseid, $groupid = 0, $returncontents = false, $search = '', $advsearch = null,
        ...
        if (!empty($params['advsearch'])) {
            ...
            list($searcharray, $params['search']) = data_build_search_array($data, false, [], $defaults);
        } else {
            $advanced = null;
            $searcharray = null;
        }
    ...
    public static function approve_entry_parameters() {
        return new external_function_parameters(
            array(
                'entryid' => new external_value(PARAM_INT, 'Record entry id.'),
                'approve' => new external_value(PARAM_BOOL, 'Whether to approve (true) or disapprove the entry.'),
            )
        );
    }

    /**
     * Approves or disapproves an entry.
     *
     * @param int $entryid          the record entry id id
     * @param bool $approve         whether to approve (true) or disapprove the entry
     * @return array of warnings and the entries
     * @since Moodle 3.3
     * @throws moodle_exception
     */
    ...
    public static function delete_entry_returns() {
        return new external_single_structure(
            array(
                'status' => new external_value(PARAM_BOOL, 'Status: true if the entry was deleted, false otherwise'),
                'warnings' => new external_warnings()
            )
        );
    }
                ...
                'data' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            'value' => new external_value(PARAM_RAW, 'The contents for the field.'),
                        )
    ...
    public static function add_entry($databaseid, $groupid, $data) {
        ...
        foreach ($params['data'] as $data) {
            $subfield = (!empty($data['subfield'])) ? '_' . $data['subfield'] : '';
        ...
        if (!empty($processeddata->fieldnotifications)) {
            foreach ($processeddata->fieldnotifications as $field => $notif) {
                $fieldnotifications[] = [
                    'fieldname' => $field,
                    'notififcation' => $notif,
                ];
    ...
    public static function add_entry_returns() {
        return new external_single_structure(
            array(
                'created' => new external_value(PARAM_BOOL, 'True if the entry was successfully created, false other wise.'),
                ...
                'fieldnotifications' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'fieldname' => new external_value(PARAM_PLUGIN, 'The field name.'),
                ...
                'data' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'fieldid' => new external_value(PARAM_INT, 'The field id.'),
                            'subfield' => new external_value(PARAM_NOTAGS, 'The subfield name (if required).', VALUE_DEFAULT, null),
                            'value' => new external_value(PARAM_RAW, 'The new contents for the field.'),
                        )
    ...
    public static function update_entry($entryid, $data) {
        ...
        $record = $DB->get_record('data_records', array('id' => $params['entryid']), '*', MUST_EXIST);
        list($database, $course, $cm, $context) = self::validate_database($entry->dataid);
        // Check database is open in time.
        data_require_time_available($database, null, $context);
