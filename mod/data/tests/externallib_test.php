...
class mod_data_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_get_entry() {
        ...
        // Now indicating my group.
        $result = mod_data_external::get_entry($entry11, $this->group1->id);
        $result = external_api::clean_returnvalue(mod_data_external::get_entry_returns(), $result);
        $this->assertCount(0, $result['warnings']);
        $this->assertEquals($entry11, $result['entry']['id']);
        ...
        $result = mod_data_external::get_entry($entry11, $this->group1->id, true);
        $result = external_api::clean_returnvalue(mod_data_external::get_entry_returns(), $result);
        $this->assertCount(0, $result['warnings']);
    ...
    /**
     * Test get_entry from other group.
     */
    public function test_get_entry_other_group() {
        list($entry11, $entry12, $entry13, $entry21) = self::populate_database_with_entries();
