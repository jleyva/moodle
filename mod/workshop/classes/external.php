...
class mod_workshop_external extends external_api {
    ...
    protected static function validate_submission($submission, workshop $workshop) {
        ...
        $canview = $canview || $submission->gradeoverby == $USER->id;   // I gave feedback.
    ...
    public static function get_assessment_form_definition($assessmentid, $mode = 'assessment') {
        ...
        $result = array(
            ...
            'descriptionfiles' => external_util::get_area_files($context->id, 'workshopform_accumulative', 'description'),
            'warnings' => $warnings
        );
                ...
                foreach ($alldata as $key => $val) {
                    if (strpos($key, 'peercomment__idx_') === 0) {
                        ...
                        list($val, $format) = external_format_text($val, $alldata['dimensionid__idx_' . $id . 'format'],
                            $context->id, 'workshopform_accumulative', 'description', $alldata['dimensionid__idx_' . $id]);
                    }
                    $result[$typeofdata][] = array(
                        'name' => $key,
                        'value' => $val
     ...
     * @since Moodle 3.4
     */
    public static function get_user_assessments_parameters() {
        return new external_function_parameters(
            array(
                'workshopid' => new external_value(PARAM_INT, 'Workshop instance id.'),
                'userid' => new external_value(PARAM_INT, 'User id (empty or 0 for current user).', VALUE_DEFAULT, 0),
    ...
    /**
     * Retrieves the assessment of the given user.
     *
     * @param int $workshopid       the workshop instance id
     ...
     * @throws moodle_exception
     */
    public static function get_user_assessments($workshopid, $userid = 0) {
        global $USER, $DB, $PAGE;

        $params = self::validate_parameters(
            self::get_user_assessments_parameters(), array('workshopid' => $workshopid, 'userid' => $userid)
        );
        $warnings = $assessments = array();
     ...
     * @since Moodle 3.4
     */
    public static function get_user_assessments_returns() {
        return new external_single_structure(
            array(
                'assessments' => new external_multiple_structure(
                    assessment_exporter::get_read_structure()
    ...
    public static function update_assessment_returns() {
        return new external_single_structure(
            array(
                'status' => new external_value(PARAM_BOOL, 'status: true if the assessment was added or updated.'),
