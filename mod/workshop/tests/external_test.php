...
class mod_workshop_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_mod_workshop_get_user_plan_student() {
        ...
        $this->assertCount(5, $result['userplan']['phases']);  // Always 4 phases.
    ...
    public function test_add_submission() {
        ...
        $options = array(
            ...
            array('name' => 'contentformat', 'value' => FORMAT_MOODLE),
            array('name' => 'inlineattachmentsid', 'value' => $draftidinlineattach),
        ...
        $result = mod_workshop_external::add_submission($this->workshop->id, $title, $options);
    ...
    public function test_update_submission() {
        ...
        $result = mod_workshop_external::update_submission($submissionid, $title, $options);
    ...
    public function test_get_submission_assessments_student() {
        ...
        foreach ($result['assessments'] as $assessment) {
            if ($assessment['grade'] == 90) {
                ...
                // Student's can see who did the review.
    ...
    public function test_get_submission_assessments_teacher() {
        ...
        $result = mod_workshop_external::get_submission_assessments($submissionid);
        $result = external_api::clean_returnvalue(mod_workshop_external::get_submission_assessments_returns(), $result);
        $this->assertEquals($this->anotherstudentg1->id, $result['assessments'][0]['reviewerid']);
    }
    ...
    public function test_get_user_assessments() {
        ...
        $this->setUser($this->student);
        // Get my assessments.
        $result = mod_workshop_external::get_user_assessments($this->workshop->id);
        $result = external_api::clean_returnvalue(mod_workshop_external::get_user_assessments_returns(), $result);
        ...
        $result = mod_workshop_external::get_user_assessments($this->workshop->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_workshop_external::get_user_assessments_returns(), $result);
        $this->assertCount(2, $result['assessments']);
    }

    /**
     * Test get_user_assessments_other_student.
     */
    public function test_get_user_assessments_other_student() {
        global $DB;
        ...
        $this->setUser($this->student);
        $this->setExpectedException('moodle_exception');
        mod_workshop_external::get_user_assessments($this->workshop->id, $this->anotherstudentg1->id);
    }

    /**
     * Test get_user_assessments_invalid_phase.
     */
    public function test_get_user_assessments_invalid_phase() {
        global $DB;
    ...
    public function test_evaluate_assessment() {
        ...
        $this->setUser($this->teacher);
        $data = array(
            array('name' => 'feedbackreviewer', 'value' => 'The feedback'),
            array('name' => 'feedbackreviewerformat', 'value' => FORMAT_MOODLE),
            array('name' => 'weight', 'value' => 25),
            array('name' => 'gradinggradeover', 'value' => 10),
        );
        $result = mod_workshop_external::evaluate_assessment($assessmentid, $data);
        $result = external_api::clean_returnvalue(mod_workshop_external::evaluate_assessment_returns(), $result);
        $this->assertTrue($result['status']);

        $result = mod_workshop_external::get_assessment($assessmentid);
        $result = external_api::clean_returnvalue(mod_workshop_external::get_assessment_returns(), $result);
        $this->assertEquals('The feedback', $result['assessment']['feedbackreviewer']);
        $this->assertEquals(25, $result['assessment']['weight']);
    }
    ...
    public function test_evaluate_assessment_ignore_parameters() {
        ...
        $data = array(
            ...
            array('name' => 'feedbackreviewerformat', 'value' => FORMAT_MOODLE),
            array('name' => 'weight', 'value' => 25),
            array('name' => 'gradinggradeover', 'value' => 1000),
        );
        $result = mod_workshop_external::evaluate_assessment($assessmentid, $data);
        $result = external_api::clean_returnvalue(mod_workshop_external::evaluate_assessment_returns(), $result);
        $this->assertTrue($result['status']);
    ...
    public function test_evaluate_assessment_no_permissions() {
        ...
        $data = array(
            ...
            array('name' => 'feedbackreviewerformat', 'value' => FORMAT_MOODLE),
            array('name' => 'weight', 'value' => 25),
            array('name' => 'gradinggradeover', 'value' => 50),
        );
        $this->expectException('moodle_exception');
        mod_workshop_external::evaluate_assessment($assessmentid, $data);
    }
