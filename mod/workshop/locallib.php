...
class workshop {
    ...
    public function validate_submission_data($data) {
        ...
        if (isset($data['attachment_filemanager']) and isset($this->submissionfiletypes)) {
            $whitelist = self::normalize_file_extensions($this->submissionfiletypes);
            if ($whitelist) {
                $draftfiles = file_get_drafarea_files($data['attachment_filemanager']);
                if ($draftfiles) {
                    $wrongfiles = array();
                    foreach ($draftfiles->list as $file) {
                        if (!self::is_allowed_file_type($file->filename, $whitelist)) {
                            $wrongfiles[] = $file->filename;
                        }
                    }
                    if ($wrongfiles) {
                        $a = array(
                            'whitelist' => self::clean_file_extensions($whitelist),
